...
\begin{proof}(Theorem)
  ...
  \begin{equation*}
    ...
    \end{gathered}
  \end{equation*}
  Now, defining $p = \bigl(\beta_i / n\bigr)^2$, and applying Lemma
  \eqref{Binomial} we get
  \begin{equation*}
    \pr\left(\sum_{t=1}^n Y_t > k \right) \leq \pr\left(B(n, p_i) > k\right).
  \end{equation*}
  Let us define the event $E_i = \left\{v_i(n) \leq \beta_i\left\}$.
  Then we can prove that
  \begin{equation*}
    \pr\left(v_{i+1}(n) > \beta_{i+1}\right) \leq
  ...
  \end{equation*}
    ...
    $\sum_{t=1}^nY_t$ counts the number of $t$ such that $h(t) \geq i$.
